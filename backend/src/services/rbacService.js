...
class RBACService {
  ...
  /**
   * Check if user can access performance data (HR dashboard, reports)
   * Only HR, Management, Department Heads, and Superadmin can access
   */
  static canAccessPerformanceData(user) {
    const allowedRoles = ['superadmin', 'management', 'department_head', 'manager'];
    return allowedRoles.includes(user.role);
  }

  /**
   * Check if user can access a specific employee's performance data
   * Employees can see their own, HR/Management can see any
   */
  static canAccessEmployeePerformance(requestingUser, targetUserId) {
    // User can always see their own performance
    if (requestingUser.id === targetUserId) {
      return true;
    }

    // HR, Management, and Superadmin can see any employee
    const allowedRoles = ['superadmin', 'management', 'department_head', 'manager'];
    return allowedRoles.includes(requestingUser.role);
  }
}
