...
function initializeSocket(server) {
  ...
  io.on('connection', (socket) => {
    ...
    /**
     * WebRTC call signaling — relay payload to target user's personal room.
     * call_id is generated by caller; both peers track it for the active call.
     */
    socket.on('call:invite', (data) => {
      try {
        const target = data && data.to_user_id;
        if (!target) return;
        io.to(`user:${target}`).emit('call:invite', {
          call_id: data.call_id,
          from_user_id: socket.userId,
          from_name: socket.user.name,
          room_id: data.room_id || null,
          media: data.media || 'audio' // 'audio' or 'video'
        });
      } catch (err) {
        logger.error('call:invite error', { error: err.message });
      }
    });
    ...
    socket.on('call:accept', (data) => {
      const target = data && data.to_user_id;
      if (!target) return;
      io.to(`user:${target}`).emit('call:accept', {
        call_id: data.call_id,
        from_user_id: socket.userId
      });
    });
    ...
    socket.on('call:reject', (data) => {
      const target = data && data.to_user_id;
      if (!target) return;
      io.to(`user:${target}`).emit('call:reject', {
        call_id: data.call_id,
        from_user_id: socket.userId,
    ...
    socket.on('call:end', (data) => {
      const target = data && data.to_user_id;
      if (!target) return;
      io.to(`user:${target}`).emit('call:end', {
        call_id: data.call_id,
        from_user_id: socket.userId
      });
    });
    ...
    socket.on('call:ice', (data) => {
      const target = data && data.to_user_id;
      if (!target) return;
      io.to(`user:${target}`).emit('call:ice', {
        call_id: data.call_id,
        from_user_id: socket.userId,
