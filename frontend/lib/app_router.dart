...
import 'auth/providers/auth_provider.dart' show authProvider, authInitializationProvider, AuthState;
...
import 'features/chat/views/chat_room_info_page.dart';
import 'features/chat/views/chat_search_page.dart';
...
final routerProvider = Provider<GoRouter>((ref) {
  ...
  return GoRouter(
    ...
    redirect: (context, state) {
      ...
      // This prevents 401 errors from premature API calls before session restoration.
      final authInitState = ref.watch(authInitializationProvider);
      
      // While auth is initializing, show loading page or stay at current location
      if (authInitState.isLoading) {
      ...
      ShellRoute(
        ...
        routes: [
          ...
          GoRoute(
            ...
            builder: (_, state) {
              final id = int.tryParse(state.pathParameters['id'] ?? '');
              if (id == null) return const NotificationListScreen();
           ...
           GoRoute(path: '/chat/search', builder: (_, __) => const ChatSearchPage()),
           GoRoute(
             path: '/chat/:id',
             builder: (_, state) {
               final id = int.tryParse(state.pathParameters['id'] ?? '');
           ...
           GoRoute(
             ...
             builder: (_, state) {
               final id = int.tryParse(state.pathParameters['id'] ?? '');
               if (id == null) return const ChatListPage();
               return ChatRoomInfoPage(roomId: id);
             },
           ),
           ...
           GoRoute(
             ...
             builder: (_, state) {
               final id = int.tryParse(state.pathParameters['id'] ?? '');
               if (id == null) return const PendingApprovalsScreen();
               return ApprovalDetailScreen(taskId: id);
             },
           ),
        ],
      ),
